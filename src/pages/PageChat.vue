<template>
  <q-page ref="pageChat" class="page-chat flex">
    <div
    ...
    >
      <q-chat-message
        ...
        :class="!otherUserDetails.online ? 'q-mt-xl' : ''"
      ...
      >
        ...
        <div class="text-body2" v-for="(msg, j) in message" :key="j">
      ...
      <q-toolbar class="full-width">
        <q-form class="full-width">
          <q-list class="full-width">
            ...
            >
              <q-item class="q-pa-none" v-if="expirationEnabled">
                <q-item-section avatar>
                  <div class="text-weight-bold text-secondary">
                    <q-icon color="secondary" name="auto_delete" size="sm" />
            ...
            <q-item class="q-pa-none" key="message_panel">
              <q-item-section>
                <q-input
                  dense
                  rounded
                  ...
                  ref="newMessage"
                  bg-color="white"
                  :disable="!otherUserId || !userDetails.passphrase"
                  v-model="newMessage.text"
                  @keyup="updateFirstChange"
                  @keydown.enter.exact.prevent="sendMessage"
                  :label="newMessage.text ? undefined : 'Message'"
                >
...
export default defineComponent({
  ...
  watch: {
    ...
    expirationEnabled: {
      handler(val) {
        if (!val) this.firstChat = ref(false);
      },
      deep: true,
    },
  },
  ...
  methods: {
    ...
    async initializeUserMessages() {
      if (this.userDetails.passphrase) {
        await this.firebaseStopGettingMessages();
        this.firebaseGetMessages({
          otherUserId: this.otherUserId,
          audioSrc: this.newMsg_NotifAudio,
        });
        if (!this.$q.platform.is.mobile) this.$refs.newMessage.focus();
        this.firstChat = ref(true);
        this.clearMessage();
      }
    },
    updateFirstChange() {
          ...
          if (plainMsg.success) {
            ...
            if (
              ...
              setTimeout(() => {
                ...
                if (this.decryptedMsg) delete this.decryptedMsg[msg.id];
              }, msg.data.expiration);
            }
            result = plainMsg.response;
          } else {
            customAlert("Failed to decrypt message", "negative");
          }
        })
        .catch((error) => {
        ...
        await this.$openpgp_methods
          ...
          .then((encryptedMsg) => {
            if (encryptedMsg.success) {
              ...
              customAlert("Failed to encrypt message", "negative");
            }
          })
          .catch((error) => {
    ...
    async removeMessage(options) {
      await this.firebaseDeleteMessage({
        ...
        .then((result) => {
          if (result.success) {
            customAlert("Message has been deleted.", "positive");
          } else {
            customAlert("Failed to delete message.", "negative");
          }
        })
        .catch((error) => {
